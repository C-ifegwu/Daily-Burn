...
class _HomeScreenState extends State<HomeScreen>
  ...
  Future<void> _showEditProfileDialog() async {
    ...
    String initialName = (user.displayName ?? '').trim();
    try {
      ...
      final String dbName = (data?['displayName'] as String? ?? '').trim();
      if (dbName.isNotEmpty) {
        initialName = dbName;
      }
    } catch (_) {
      // Keep auth name fallback when Firestore read fails.
    }
    ...
    final String safeName = updatedName.trim();
    if (safeName.isEmpty) {
      if (!mounted) {
        return;
      }
    ...
    await _saveProfileName(safeName);
  }

  Future<void> _saveProfileName(String displayName) async {
    final User? user = FirebaseAuth.instance.currentUser;
    if (user == null) {
      return;
    }
  ...
  Future<void> _handleProfileAction(String value) async {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(content: Text('Unknown profile action.')),
    );
  }
      ...
      builder: (context, budget, _) {
        ...
        return StreamBuilder<DocumentSnapshot<Map<String, dynamic>>>(
          ...
          builder: (context, snapshot) {
            ...
            final String greetingName = displayName.isNotEmpty
                ? displayName
                        ...
                        child: Column(
                          children: [
                            ...
                            AnimatedBuilder(
                              ...
                              builder: (_, __) =>
                                  _buildCircle(budget, stateColor),
                            ),
          ...
          children: [
            ...
            Text(label,
                style: GoogleFonts.inter(
                    fontSize: 14,
                    fontWeight: FontWeight.w600,
                    color: isOutline ? AppTheme.textPrimary : Colors.white)),
          ],
        ),
      ),
    );
    ...
    return Container(
      ...
      child: Column(
        children: [
          ...
          const SizedBox(height: 4),
          Text('Tap + Add Expense to log your first spend.',
              style: AppTheme.bodyMedium.copyWith(fontSize: 13),
